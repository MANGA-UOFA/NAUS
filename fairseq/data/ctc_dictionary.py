...
class CTCDictionary:
    """
    A customized dictionary to include an extra CTC blank token
    """
    def __init__(
        self,
    ...
    def __getitem__(self, idx):
        if idx < len(self.symbols):
            return self.symbols[idx]  # We added extra 1 since we included an extra blank token
    ...
    def add_file_to_dictionary(filename, dict, tokenize, num_workers):
        ...
        if num_workers > 1:
            ...
            for (start_offset, end_offset) in chunks:
                results.append(
                    pool.apply_async(
                        CTCDictionary._add_file_to_dictionary_single_worker,
                        (
                            local_file,
                            tokenize,
                            dict.eos_word,
