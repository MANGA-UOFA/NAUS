...
class TransformerEncoderConfig(FairseqDataclass):
    # TODO: Reorganize the encoder config files with incoherence
    embed_path: Optional[str] = field(
        default=None, metadata={"help": "path to pre-trained embedding"}
    )
    embed_dim: Optional[int] = field(
    ...
    # Notice we do not have the max_target_positions parameter for this encoder config since max_target_positions should
    # be the same as the max_source_positions in this encoder only setting.
    share_all_embeddings: bool = field(
        default=False,
        metadata={
            "help": "share the input and output embeddings of the encoder"
        },
    )
    ...
    encoder_learned_pos: bool = field(
        default=False,
        metadata={
            "help": "whether to learn encoder pos"
        },
    )
